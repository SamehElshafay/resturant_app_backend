...
@section('content')
    <div class="card p-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0">New Purchase Invoice</h4>
        </div>
        ...
        <form action="{{ route('purchase_invoices.store') }}" method="POST">
            ...
            <div class="row g-3">
                    ...
                    <select name="supplier_id" class="form-select @error('supplier_id') is-invalid @enderror">
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" 
                                {{ (isset($duplicateFrom) && $duplicateFrom->supplier_id == $supplier->id) ? 'selected' : '' }}>
                ...
                <!-- Branch Removed: Items stored in Central Warehouse -->
                <div class="col-md-12">
                    <label class="form-label">Notes</label>
                    <textarea name="notes" class="form-control" rows="1">{{ isset($duplicateFrom) ? $duplicateFrom->notes : '' }}</textarea>
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody id="items-table">
                        @if(isset($duplicateFrom))
                            @foreach($duplicateFrom->items as $index => $item)
                                <tr>
                                    <td>
                        ...
                        @else
                            <tr>
                                <td>
                                    <select name="items[0][ingredient_id]" class="form-control product-select"
                                        style="color: #000;">
                                        <option value="" class="text-muted">Select Ingredient...</option>
                                        @foreach($ingredients as $ingredient)
                                ...
                                <td>
                                    <input type="number" name="items[0][quantity]" class="form-control qty" min="0.001"
                                        step="0.001" value="1">
                                </td>
                                <td>
                                    <input type="number" name="items[0][unit_price]" class="form-control price" min="0"
                                        step="0.01" value="0">
                                </td>
                                <td>
                ...
                <div class="text-end mt-3">
                    <h4>Total: <span id="grand-total">{{ isset($duplicateFrom) ? number_format($duplicateFrom->total_amount, 2, '.', '') : '0.00' }}</span></h4>
                </div>
            </div>
